...
from dr_streamlit import multiclass_dropdown_menu, derived_features_chart, text_feature_dropdown_menu, \
    project_model_dropdown, get_deployment, get_project, create_prediction_form, \
...
def main():
    try:
        ...
        page_names_to_funcs = {
            "Overview": intro,
            "Model Insights": insights_app,
        }
        if deployment_id:
            page_names_to_funcs["Model Predictions"] = predictor_app

        st.sidebar.markdown(
            datarobot_logo(),
            unsafe_allow_html=True,
        )
        demo_name = st.sidebar.selectbox("Discover", page_names_to_funcs.keys())
